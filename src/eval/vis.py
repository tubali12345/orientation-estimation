...
class Visualizer:
    ...
    def plot_hist_error(self, errors: np.ndarray, bins: int = 100):
        ...
        plt.legend()
        plt.savefig(f"{self.output_dir}/angular_error_hist.png", bbox_inches="tight")
        plt.show()

    def plot_scatter_true_pred(self, y_true: np.ndarray, y_pred: np.ndarray):
        plt.figure(figsize=(6, 6))
        plt.scatter(y_true, y_pred, alpha=0.5)
        plt.plot([0, 360], [0, 360], "r", linestyle="--")
        plt.xlabel("Target Angle (degrees)")
        plt.ylabel("Predicted Angle (degrees)")
        plt.title("Predicted vs. Target Angles")
        plt.xlim(0, 360)
        plt.ylim(0, 360)
        plt.savefig(f"{self.output_dir}/true_vs_pred_scatter.png")
        plt.show()

    def plot_scatter_dist_error(
        self, mic_positions: np.ndarray, src_positions: np.ndarray, errors: np.ndarray, bin_width: float = 0.5
    ):
        distances = self.positions_to_distance(mic_positions, src_positions)
        ...
        plt.legend()
        plt.savefig(f"{self.output_dir}/angular_error_vs_distance.png", bbox_inches="tight")
        plt.show()

    def plot_grid_micpos_vs_error(
        self, mic_positions: np.ndarray, errors: np.ndarray, room_length: float = 5.0, room_width: float = 5.0
    ):
        ...
        plt.savefig(f"{self.output_dir}/micpos_vs_error.png")
        plt.show()

    def plot_grid_srcpos_vs_error(
        self, src_positions: np.ndarray, errors: np.ndarray, room_length: float = 5.0, room_width: float = 5.0
    ):
        fig, ax = plt.subplots(figsize=(8, 8))
        ...
        plt.savefig(f"{self.output_dir}/srcpos_vs_error.png")
        plt.show()

    def plot_degree_vs_error(self, y_true: np.ndarray, errors: np.ndarray, resolution: int = 10):
        ...
        theta = np.deg2rad(degree_bins + resolution / 2)
        ...
        plt.savefig(f"{self.output_dir}/degree_vs_error.png", bbox_inches="tight")
        plt.show()

    def plot_error_spider_chart(self, y_true: np.ndarray, errors: np.ndarray, resolution: int = 10):
        degree_bins = np.arange(0, 360, resolution)
        error_per_bin = np.zeros(len(degree_bins))
        counts = np.zeros(len(degree_bins))
        ...
        plt.savefig(f"{self.output_dir}/spider_chart_error.png", bbox_inches="tight")
        plt.show()
    ...
    def positions_to_distance(mic_positions: np.ndarray, src_positions: np.ndarray) -> np.ndarray:
        return np.array(
            [Visualizer.euclidean_distance(mic_pos, src_pos) for mic_pos, src_pos in zip(mic_positions, src_positions)]
        )
